...
- 重试未完成的部署时，默认恢复上次确认的镜像、应用、站点和网络输入；数据库密码仍沿用原值；
...
检测到已有部署时，脚本会读取 `deployment.state`、`.env` 和上次确认时写入的
`deployment.inputs`。若上次部署未完成，`deployment.inputs` 中的非敏感输入优先作为
重试默认值：

- 应用镜像默认使用上次确认的版本；没有未完成的部署时才使用当前已部署版本；
- PostgreSQL 容器或本机部署方式自动沿用，不会重复询问；
- 数据库密码默认保持不变；
- `frappe_docker` 默认使用上次确认的 ref；没有未完成的部署时固定到上次部署的准确 commit，不会自动跟随 `main`；
...
`.env`，它依赖服务器 root 权限保护，不能替代独立的密钥管理服务。
